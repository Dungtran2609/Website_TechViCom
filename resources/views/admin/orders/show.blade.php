                ...
                @endforeach
                        ...
                        <div class="mt-4">
                            ...
                            @elseif($orderData['status'] === 'delivered')
                                {{-- Chỉ cho xác nhận thanh toán khi đã nhận được hàng và chưa thanh toán --}}
                            ...
                            <form action="{{ route('admin.orders.destroy', $orderData['id']) }}" method="POST" class="d-inline ms-2"
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" @if(!in_array($orderData['status'], haystack: ['cancelled', 'returned', 'received'])) disabled @endif>
