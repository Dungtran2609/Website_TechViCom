...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.products.brands.index') }}"
                class="d-flex gap-2 align-items-center flex-wrap">
                <input type="text" name="search" value="{{ request('search') }}" class="form-control w-auto"
                    style="min-width:180px;max-width:220px;" placeholder="Tìm tên thương hiệu...">
                    ...
                    <tbody>
                        @forelse ($brands as $brand)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge bg-{{ $brand->status ? 'success-subtle text-success' : 'danger-subtle text-danger' }}">
                                ...
                                <td class="text-center">
                                    <div class="d-flex gap-2 justify-content-center">
                                        ...
                                        <form action="{{ route('admin.products.brands.destroy', $brand) }}" method="POST"
                                            class="d-inline"
                                            onsubmit="return confirm('Bạn có chắc muốn chuyển thương hiệu này vào thùng rác?')">
                                            @csrf
                                            @method('DELETE')
