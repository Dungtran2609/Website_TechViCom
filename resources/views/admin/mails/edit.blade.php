        ...
        <form action="{{ route('admin.mails.update', $mail->id) }}" method="POST">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                               value="{{ old('name', $mail->name) }}" placeholder="Nhập tên mẫu mail...">
                        @error('name')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select name="type" class="form-select @error('type') is-invalid @enderror">
                            ...
                            <option value="order" {{ old('type', $mail->type) == 'order' ? 'selected' : '' }}>Đơn hàng</option>
                            <option value="user" {{ old('type', $mail->type) == 'user' ? 'selected' : '' }}>Người dùng</option>
                            <option value="system" {{ old('type', $mail->type) == 'system' ? 'selected' : '' }}>Hệ thống</option>
                            <option value="promotion" {{ old('type', $mail->type) == 'promotion' ? 'selected' : '' }}>Khuyến mãi</option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="mb-3">
                ...
                <input type="text" name="subject" class="form-control @error('subject') is-invalid @enderror" 
                       value="{{ old('subject', $mail->subject) }}" placeholder="Nhập tiêu đề mail...">
                @error('subject')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="content" id="content-textarea" class="form-control @error('content') is-invalid @enderror" 
                          rows="12" placeholder="Nhập nội dung mail...">{{ old('content', $mail->content) }}</textarea>
                @error('content')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <div class="form-text">
                ...
                <div class="col-md-6">
                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" 
                               {{ old('is_active', $mail->is_active) ? 'checked' : '' }}>
                ...
                <div class="col-md-6">
                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" name="auto_send" value="1" 
                               {{ old('auto_send', $mail->auto_send) ? 'checked' : '' }}>
                        <label class="form-check-label fw-bold">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('admin.mails.show', $mail->id) }}" class="btn btn-info">
