            ...
            <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST">
                    ...
                    <div class="border rounded p-2" style="max-height:220px;overflow:auto;">
                        @foreach($coupons as $coupon)
                            <div class="form-check">
                                <input class="form-check-input coupon-checkbox" type="checkbox" name="coupons[]" id="coupon_{{ $coupon->id }}" value="{{ $coupon->id }}" {{ $promotion->coupons->contains('id', $coupon->id) ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <select name="type" id="type" class="form-select" required>
                        <option value="all" {{ $promotion->type == 'all' ? 'selected' : '' }}>Toàn bộ sản phẩm</option>
                        <option value="category" {{ $promotion->type == 'category' ? 'selected' : '' }}>Theo danh mục</option>
                        <option value="product" {{ $promotion->type == 'product' ? 'selected' : '' }}>Theo sản phẩm</option>
                    </select>
                </div>

                {{-- Chọn danh mục --}}
                <div class="mb-3" id="category-select" style="display:{{ $promotion->type == 'category' ? 'block' : 'none' }};">
                ...
                {{-- Chọn sản phẩm --}}
                <div class="mb-3" id="product-select" style="display:{{ $promotion->type == 'product' ? 'block' : 'none' }};">
                    ...
                    <div class="border rounded p-2" style="max-height:220px;overflow:auto;">
                        @foreach($products as $product)
                            <div class="form-check">
                                <input class="form-check-input product-checkbox" type="checkbox" name="products[]" id="product_{{ $product->id }}" value="{{ $product->id }}" {{ $promotion->products->contains($product->id) ? 'checked' : '' }}>
