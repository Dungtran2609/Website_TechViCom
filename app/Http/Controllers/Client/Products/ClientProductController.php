...
class ClientProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('category')) {
            $categorySlugs = is_array($request->category) ? $request->category : explode(',', $request->category);
            // Lấy danh mục đầu tiên để hiển thị tên
            $firstSlug = $categorySlugs[0];
            if (!is_numeric($firstSlug)) {
                $currentCategory = Category::where('slug', $firstSlug)->first();
            }
        }
    ...
    public function filterProducts(Request $request)
    {
        ...
        $html = view('client.products.partials.product-grid', compact('products', 'favoriteProductIds'))->render();
