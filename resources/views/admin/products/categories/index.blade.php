                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td>
                                @if ($category->image)
                                    <div class="rounded bg-light avatar-md d-flex align-items-center justify-content-center">
                                        <img src="{{ asset('storage/' . $category->image) }}" alt="Image" class="avatar-md">
                                    </div>
                                @else
                                    Không có ảnh.
                                @endif
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    <a href="{{ route('admin.products.categories.show', $category) }}" class="btn btn-light btn-sm" title="Xem chi tiết">
                                        <iconify-icon icon="solar:eye-broken" class="align-middle fs-18"></iconify-icon>
                                    </a>
                                    <a href="{{ route('admin.products.categories.edit', $category) }}" class="btn btn-soft-primary btn-sm" title="Chỉnh sửa">
                                        <iconify-icon icon="solar:pen-2-broken" class="align-middle fs-18"></iconify-icon>
                                    </a>
                                    <form action="{{ route('admin.products.categories.destroy', $category) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
