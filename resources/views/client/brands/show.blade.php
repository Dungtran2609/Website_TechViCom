...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        {{-- Card thương hiệu --}}
        <div
            class="bg-white rounded-2xl shadow-lg p-6 mb-10 flex flex-col md:flex-row items-center gap-6 transition hover:shadow-xl">
            {{-- Ảnh thương hiệu --}}
            ...
            <div class="flex-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-8 text-gray-700">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <span
                            class="px-3 py-1 rounded-full text-xs font-semibold {{ $brand->status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
            ...
            @forelse ($products as $product)
                <div class="bg-white rounded-xl shadow hover:shadow-lg transition cursor-pointer group overflow-hidden"
                    ...
                    <div class="relative">
                        ...
                        {{-- Badge HOT --}}
                        <span
                            class="absolute top-2 left-2 bg-green-600 text-white px-2 py-1 rounded text-xs font-bold">HOT</span>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                @if ($product->type === 'simple' && $product->variants->count() > 0)
                                    ...
                                    <span
                                        class="text-lg font-bold text-orange-500">{{ number_format($variant->price) }}₫</span>
                                @elseif($product->type === 'variable' && $product->variants->count() > 0)
                                    ...
                                    @if ($minPrice === $maxPrice)
                                        <span
                                            class="text-lg font-bold text-orange-500">{{ number_format($minPrice) }}₫</span>
                                    @else
                                        <span class="text-lg font-bold text-orange-500">{{ number_format($minPrice) }} -
