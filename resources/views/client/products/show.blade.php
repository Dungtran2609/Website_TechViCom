                        ...
                        <div class="flex items-center gap-4 pt-4 border-t border-gray-200">
                            ...
                            <div class="flex items-center">
                                <button type="button" id="quantity-minus-btn"
                                    onclick="updateQuantity(-1, {{ $product->id ?? 0 }})"
                            ...
                            <div
                                ...
                                <a href="{{ route('products.show', $relatedProduct->id) }}">
                                    <div class="aspect-square bg-gray-50">
                                        <img src="{{ $relatedProduct->thumbnail ? asset('storage/' . $relatedProduct->thumbnail) : asset('client_css/images/placeholder.svg') }}"
                                    ...
                                    <div class="p-4">
                                        ...
                                        <p class="text-[#ff6c2f] font-bold">
                                            @if ($relatedProduct->type === 'simple' && $relatedProduct->variants->count() > 0)
                                                @php $variant = $relatedProduct->variants->first(); @endphp
                                                {{ number_format($variant->price) }}₫
                                            @elseif($relatedProduct->type === 'variable' && $relatedProduct->variants->count() > 0)
                                                @php
                    ...
                    @if ($canComment)
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
                            ...
                            <form action="{{ route('products.comments.store', $product->id) }}" method="POST">
                                @csrf
                                <div class="mb-4">
                                    <label for="rating" class="block text-sm font-medium text-gray-700 mb-2">Đánh
                                        giá</label>
                                    <div class="flex items-center space-x-2">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <input type="radio" id="rating_{{ $i }}" name="rating"
                                ...
                                <div class="mb-4">
                                    <label for="content" class="block text-sm font-medium text-gray-700 mb-2">Nội dung bình
                                        luận</label>
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-blue-700 text-sm">
                                        @if (!auth()->check())
                                            Bạn cần <a href="{{ route('login') }}" class="underline">đăng nhập</a> để bình
                                            luận.
                                        @else
                                ...
                                <div class="flex items-start space-x-4">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center space-x-2 mb-2">
                                            ...
                                            <span
                                                class="text-gray-500 text-sm">{{ $comment->created_at->format('d/m/Y H:i') }}</span>
                                        </div>
                                        ...
                                        @if ($comment->rating)
                                            <div class="flex items-center mb-2">
                                                @for ($i = 1; $i <= 5; $i++)
                                                    <i
                                                        class="fas fa-star text-{{ $i <= $comment->rating ? 'yellow' : 'gray' }}-400"></i>
                                                @endfor
                                        ...
                                        @auth
                                            ...
                                            @if ($canReply)
                                                ...
                                                <div id="reply-form-{{ $comment->id }}" class="hidden mt-4">
                                                    <form action="{{ route('products.comments.reply', $comment->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <div class="mb-3">
                                                                ...
                                                                <div class="flex-1">
                                                                    <div class="flex items-center space-x-2 mb-1">
                                                                        <h5 class="font-medium text-gray-900 text-sm">
                                                                            {{ $reply->user->name }}</h5>
                                                                        <span
                                                                            class="text-gray-500 text-xs">{{ $reply->created_at->format('d/m/Y H:i') }}</span>
                                                                    </div>
                                                                    <p class="text-gray-700 text-sm">{{ $reply->content }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateInfoDisplay() {
                ...
                if (state.activeVariant) {
                    ...
                    infoContainer.innerHTML =
                        '<span class="text-red-500 font-semibold">Vui lòng chọn phiên bản</span>';
                    return;
                }
