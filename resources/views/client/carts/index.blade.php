...
function updateQuantity(id, q){
  ...
  fetch(`/carts/${id}`,{method:'PUT',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':getCsrf()},body:JSON.stringify({quantity:q})})
...
function initSelectionFeatures(){
  ...
  // KHÔNG tick chọn tất cả mặc định nữa
...
function proceedToCheckoutSelected(){
  let ids = Array.from(document.querySelectorAll('.item-checkbox:checked')).map(cb=>cb.value);
  if(ids.length===0){ showNotification('Vui lòng chọn sản phẩm để thanh toán','error'); return; }
