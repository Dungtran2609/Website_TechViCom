<?php
/**
 * Test VNPAY Final - Logic tạo chữ ký chính xác nhất
 * Truy cập: http://localhost/vnpay_final_test.php
 */

echo "<h1>Test VNPAY Final - Logic chính xác nhất</h1>";
...
// Tạo input data với thứ tự chuẩn
$inputData = array(
    "vnp_Version" => "2.1.0",
...
echo "<p><a href='" . htmlspecialchars($vnp_Url) . "' target='_blank' style='background: #28a745; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-size: 16px; font-weight: bold;'>Test VNPAY Payment (Final)</a></p>";
...
// Test với URL đơn giản nhất
echo "<h2>Test với URL đơn giản nhất</h2>";
$simpleUrl = "https://sandbox.vnpayment.vn/paymentv2/vpcpay.html?vnp_Amount=2000000&vnp_Command=pay&vnp_CreateDate=" . date('YmdHis') . "&vnp_CurrCode=VND&vnp_IpAddr=127.0.0.1&vnp_Locale=vn&vnp_OrderInfo=Thanh+toan+don+hang+test&vnp_OrderType=other&vnp_ReturnUrl=http%3A%2F%2Flocalhost%2Fvnpay%2Freturn&vnp_TmnCode={$vnp_TmnCode}&vnp_TxnRef={$vnp_TxnRef}&vnp_Version=2.1.0&vnp_SecureHash={$vnpSecureHash}";
echo "<p><a href='" . htmlspecialchars($simpleUrl) . "' target='_blank' style='background: #dc3545; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>Test VNPAY Payment (Simple URL)</a></p>";
