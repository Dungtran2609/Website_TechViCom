<?php
/**
 * Test VNPAY Complete Fix - Kiểm tra VNPAY với fix hoàn chỉnh
 * Truy cập: http://localhost/test_vnpay_complete_fix.php
 */

echo "<h1>Test VNPAY Complete Fix</h1>";
...
// Tạo query string và hash data (FIXED VERSION)
$query = "";
$hashdata = "";
...
echo "<textarea style='width: 100%; height: 60px; font-size: 12px; font-family: monospace;'>" . htmlspecialchars($query) . "</textarea>";
...
echo "<p><a href='" . htmlspecialchars($vnp_Url) . "' target='_blank' style='background: #28a745; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-size: 16px; font-weight: bold;'>Test VNPAY Payment (Complete Fix)</a></p>";
...
$minSecureHash = hash_hmac('sha512', $minHashdata, $vnp_HashSecret);
$minUrl = $vnp_Url . "?" . $minQuery . '&vnp_SecureHash=' . $minSecureHash;

echo "<p><strong>Minimal Hash Data:</strong></p>";
echo "<textarea style='width: 100%; height: 60px; font-size: 12px;'>" . htmlspecialchars($minHashdata) . "</textarea>";
